    ...
    public function checkout(){
        ...
        $info = array( 
            ...
            'redirecturl' =>$_GET['redirecturl'],
             ...
             $id=DB::table('merchant_informations')->insert($info);
        // echo"<pre>";
        // print_r($info);
        // die();
    ...
    public function verify() {
       $order_id = $_GET['order_id'];
        ...
        if(!empty($arr->token))
        {
            ...
            $arr = array_shift($arr);   
            $spinfo = array(               
                'sp_order_id' =>$arr['order_id'],
                'sp_bank_trx_id'=>$arr['bank_trx_id'],
                'sp_bank_status' =>$arr['bank_status'],
                'sp_payment_method'=>$arr['method'],
                'sp_invoice_no' =>$arr['invoice_no'],
                'sp_code'=>$arr['sp_code'],
                'sp_message'=>$arr['sp_massage'],       
                 );
                 $tx_id=$arr['value1'];
                 DB::table('merchant_informations')->where(['value1'=>$tx_id])->update($spinfo);
